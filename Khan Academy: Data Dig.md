## Problem
For the chosen dataset (List of Solar System Objects by Size), use advanced SELECT queries to discover things about the data. What sort of questions might one have about the data?
...
/* How many objects have more than 5 times the surface gravity of Earth? */
SELECT surface_gravity_rel, COUNT(*)
	FROM solar_system_objects
	WHERE surface_gravity_rel > 3
	;

/* How many stars, planets, dwarf planets, and moons are there? */
SELECT
	CASE
		WHEN type_of_object LIKE "%star%" THEN "star"
		WHEN type_of_object LIKE "%planet%" AND type_of_object NOT LIKE "%dwarf%" THEN "planet"
		WHEN type_of_object LIKE "%dwarf%" THEN "dwarf planet"
		WHEN type_of_object LIKE "%satellite%" THEN "moon"
		ELSE "Other"
	END AS "generic_type"
	, COUNT(*) AS "quantity"
    FROM solar_system_objects
...
SELECT body, MAX(volume)
	FROM solar_system_objects
	WHERE type_of_object LIKE "%planet%" AND type_of_object NOT LIKE "%dwarf%"
	;

/* Which non-planetary object has the greatest mass? */
SELECT body, MIN(mass)
	FROM solar_system_objects
	WHERE type_of_object NOT LIKE "%planet%"
	;

/* How many moons does each planet have? NOTE: the result of this query is a table that has a null row in it (see picture below). I don't know how to get rid of this row */
SELECT
	CASE
		WHEN type_of_object = "satellite of Mercury" THEN "Mercury"
		WHEN type_of_object = "satellite of Venus" THEN "Venus"
		WHEN type_of_object = "satellite of Earth" THEN "Earth"
		WHEN type_of_object = "satellite of Mars" THEN "Mars"
		WHEN type_of_object = "satellite of Jupter" THEN "Jupiter"
		WHEN type_of_object = "satellite of Saturn" THEN "Saturn"
		WHEN type_of_object = "satellite of Uranus" THEN "Uranus"
		WHEN type_of_object = "satellite of Neptune" THEN "Neptune"
	END AS "planet_name"
	, COUNT(*) AS number_of_moons
	FROM solar_system_objects
