...
The following SQL statements create a database of company employees with referential constraints, query the database, create a view to list the employees in each department, and develop a procecdure to create a budget report table for department salary increases.
...
-- For each department, list the first and last name of each employee who is working in the department along with the first and last name of his or her immediate supervisor, and the department number and name. List all departments, including ones with no employees, and list all employees, including ones who do not have any supervisors. List results in order of each department number and the first name of each employee
...
-- Give the last name and SSN of those employees who work on any project(s) where there are more female than male employees
...
/* STORED PROCEDURE WHICH CREATES A BUDGET REPORT TABLE AND PERFORMS THE FOLLOWING ACTIONS: If Dept = 1, increase total salary by 10%. If Dept = 4, increase total salary by 20%. If Dept = 5, increase total salary by 30%. If Dept = 7, increase total salary by 40% */
